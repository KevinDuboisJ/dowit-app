...
<div
    ...
    x-data="table"
    @class([ 'fi-ta' , 'animate-pulse'=> $records === null,
    ])
    >
    <x-filament-tables::container>
        <div
            ...
            <div
                ...
                @class([ 'fi-ta-filters-above-content-ctn grid px-4 py-4 sm:px-6' ,
                ])>
        ...
        <div
            ...
            @endif
            @class([ 'fi-ta-content relative divide-y divide-gray-200 overflow-x-auto dark:divide-white/10 dark:border-t-white/10' , '!border-t-0'=> ! $hasHeader,
            ])
            >
            ...
            <x-filament::grid
                ...
                @class([ 'fi-ta-content-grid gap-4 p-4 sm:px-6'=> $contentGrid,
                ...
                <x-filament-tables::group.header
                    ...
                    @class([ 'col-span-full' , '-mx-4 w-[calc(100%+2rem)] border-y border-gray-200 first:border-t-0 dark:border-white/5 sm:-mx-6 sm:w-[calc(100%+3rem)]'=> $contentGrid,
                    ])
                ...
                <div
                    ...
                    x-sortable-handle
                    @endif
                    @class([ 'fi-ta-record relative h-full bg-white transition duration-75 dark:bg-gray-900' , 'hover:bg-gray-50 dark:hover:bg-white/5'=> ($recordUrl || $recordAction) && (! $contentGrid),
                    ...
                    <div
                        @class([ 'flex items-center' , 'ps-1 sm:ps-3'=> (! $contentGrid) && $hasItemBeforeRecordContent,
                        ...
                        <div
                            @class([ 'flex w-full flex-col gap-y-3 py-4' , 'md:flex-row md:items-center'=> ! $contentGrid,
                            ])
                            >
                            <div class="flex-1">
                                @if ($recordUrl)
            ...
            <x-filament-tables::table
                ...
                <x-slot name="headerGroups">
                    ...
                    <th
                        ...
                        <div
                            @class([ 'flex w-full items-center' , 'whitespace-nowrap'=> ! $isColumnGroupHeaderWrapped,
                ...
                <x-slot name="header">
                    ...
                    <th
                        ...
                        class="fi-ta-actions-header-cell w-1"></th>
                    @endif
                    @endif
                ...
                <tbody x-data="{ expanded: false }">
                    ...
                    <x-filament-tables::row>
                        ...
                        @if (count($actions) && $actionsPosition === ActionsPosition::BeforeCells)
                        <td
                            class="bg-gray-50 dark:bg-white/5"></td>
                        @endif
                    ...
                    <x-filament-tables::row
                        ...
                        @class([ 'group cursor-move'=> $isReordering,
                        ...$getRecordClasses($record),
                        ])
                        >
                        ...
                        $tasksIsNotEmpty = $record->tasks->isNotEmpty(); // CODE FOR SUBTABLE
                        ...
                        <x-filament-tables::cell
                            ...
                            <div class="flex items-center">
                                ...
                                <x-filament-tables::columns.column
                                    ...
                                    @class([ 'pl-[21px]'=> $loop->first && !$tasksIsNotEmpty
                                    ])/>
                    ...
                    <tr
                        ...
                        <td colspan="{{ $columnsCount }}" class="p-0 pb-7">
                            @if ($tasksIsNotEmpty)
                            <!-- Simulated inner top shadow -->
                            <div class="relative">
                                <div class="absolute top-0 left-0 w-full h-2 bg-gradient-to-b from-black/10 to-transparent pointer-events-none z-10"></div>
                                <table class="w-full divide-y divide-gray-200 dark:divide-white/5 text-sm p-4 border-collapse border-spacing-0">
                                    <thead>
                                        <tr class="bg-gray-50 dark:bg-white/5">
                                            <th class="italic text-xs text-left pl-4 font-normal pt-3" colspan="100%">Actieve en geplande taken</th>
                                        </tr>
                                        <tr class="h-10 bg-gray-50 dark:bg-white/5">
                                            <th class="text-left pl-16 pr-8 py-2">Tijdstip</th>
                                            <th class="text-left pl-16 pr-8 py-2">Status</th>
                                            <th class="text-right pl-16 pr-8 py-2 w-full"></th>
                                        </tr>
                                    </thead>
                                    <tbody class="divide-y divide-gray-200 dark:divide-white/5">
                                        @foreach ($record->tasks as $task)
                                        @php
                                        $taskStatusEnum = TaskStatusEnum::from($task->status->id);
                                        @endphp
                                        <tr>
                                            <td class="pl-16 pr-8 py-2 whitespace-nowrap">{{ $task->start_date_time }}</td>
                                            <td class="pl-16 pr-8 py-2 whitespace-nowrap">{{ $taskStatusEnum->getLabel() }}</td>
                                            @if($taskStatusEnum === TaskStatusEnum::Scheduled)

                                            <td
                                                x-data="{ loading: false, taskId: {{ $task->id }}, action: null }"
                                                x-on:show-edit-task-loading.window="if ($event.detail.id === taskId) { loading = true; action = $event.detail.action }"
                                                x-on:hide-edit-task-loading.window="if ($event.detail.id === taskId) loading = false"
                                                class="pl-16 pr-8 py-2 whitespace-nowrap">
                                                <div
                                                    class="flex items-center justify-end space-x-2 cursor-pointer
                                                    text-sm text-gray-700 dark:text-gray-200 group-hover/link:underline group-focus-visible/link:underline">
                                                    <template x-if="loading">
                                                        <x-filament::loading-indicator
                                                            class="h-5 w-5"
                                                            x-bind:class="action === 'edit' ? 'text-gray-700' : 'text-red-600'" />
                                                    </template>

                                                    <span
                                                        class=""
                                                        x-on:click="$dispatch('show-edit-task-loading', { id: taskId, action: 'edit' }); $wire.openEditTaskModal(taskId)">
                                                        Bewerken
                                                    </span>

                                                    <span
                                                        class="text-red-600"
                                                        x-on:click="$dispatch('show-edit-task-loading', { id: taskId, action: 'delete' }); $wire.openDeleteTaskModal(taskId)">
                                                        Verwijderen
                                                    </span>
                                                </div>
                                            </td>
                                ...
                                <!-- Simulated inner bottom shadow -->
                                <div class="absolute -bottom-7 left-0 w-full h-2 bg-gradient-to-t from-black/5 to-transparent pointer-events-none z-10"></div>
                            </div>
                            @endif
                        </td>
                    </tr>
    ...
    <x-filament::modal id="edit-task-modal" width="md">
        <form wire:submit="updateTask">
            {{ $this->form }}

            <x-filament::button type="submit" class="mt-4" wire:loading.attr="disabled" wire:target="updateTask">
                Bewerken
            </x-filament::button>
        </form>
    </x-filament::modal>
        ...
        <form wire:submit="deleteTask">
            ...
            <div class="flex items-center space-x-2">
                <x-filament::button
                    class="w-1/2"
                    color="gray"
                    x-on:click="$dispatch('close-modal', { id: 'delete-task-modal' })"
                    wire:loading.attr="disabled"
