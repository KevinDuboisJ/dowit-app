...
export const TaskOverviewSheet = ({
  ...
  const currentTask = useMemo(() => {
    return tasks?.data.find(task => task.id === sheetState.taskId)
  ...
  return (
    <Sheet open={sheetState.open} onOpenChange={handleSheetClose}>
      <SheetContent className="p-0 h-full bg-app-background-secondary w-full md:w-[720px] sm:max-w-screen-md">
        <SheetTitle className="sr-only">Taak details</SheetTitle>
        <SheetDescription className="sr-only">
          Details en bewerking van een taak
        </SheetDescription>
