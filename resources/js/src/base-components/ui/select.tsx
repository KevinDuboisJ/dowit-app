...
const SelectTrigger = React.forwardRef<
  ...
  SelectTriggerProps
>(({ className, children, value, onClear, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ref={ref}
    className={cn(
      "flex h-8 w-full items-center justify-between whitespace-nowrap rounded-sm border border-input bg-transparent px-3 py-2 text-xs shadow-xs placeholder:text-xs placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-ring disabled:cursor-not-allowed disabled:opacity-50 [&>span]:line-clamp-1",
      className
    )}
    {...props}
  >
    ...
    <div className="flex items-center space-x-1">
      <SelectPrimitive.Icon asChild>
        {value && (
          ...
          <GrFormClose className="h-4 w-4 opacity-50" onPointerDown={(e) => {
            e.preventDefault()
            e.stopPropagation()
          }}
            onClick={(e) => {
